...
import { HttpUnprocessedEntityError } from '../../../adapters/exceptions-adapters';
        ...
        return (req: Request, res: Response, next: NextFunction) => {
            ...
            if (schema.body) {
                ...
                if (!validatedData.error) {
                    req.body = { ...validatedData.value, ...{ _originalData: req.body } };
                }
            ...
            if (valid) {
                ...
                const details = (validateResult.bodyError ? validateResult.bodyError.details : []).concat(validateResult.parameterError ? validateResult.parameterError.details : []).concat(validateResult.queryError ? validateResult.queryError.details : []);
                const message = details.map((i) => i.message).join('; \n ');
    
                console.log(`Router validation error: ${message}`);
